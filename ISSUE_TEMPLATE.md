...
Please use this checklist for bug reports: - \[ \] [Write a minimal
reproducible example](http://stackoverflow.com/a/5963610) - \[ \]
Include example data in the minimal reproducible example - \[ \] run
...
\`\`\`{R} rm(list=ls(all=T)) gc(reset=T) set.seed(1L)

dat \<- caret::twoClassSim(100L) X \<- dat\[,1L:5L\] y \<-
dat\[\[“Class”\]\]
...
Please cut/paste the output. If your version of caret or caretEnsemble
