...
models.reg <- list(rf1, lm1, glm1, nn1)
class(models.reg) <- "caretList"

devtools::use_data(models.reg, Y.reg, X.reg, overwrite=TRUE)
...
models.class <- list(rf1, glm1, svm1, nnet1, bag1, nn1)
class(models.class) <- "caretList"

devtools::use_data(models.class, Y.class, X.class, overwrite=TRUE)

rm(list=ls())

## Uglier data with class imbalance

devtools::install_github("jknowles/EWStools@8894fcd487833f8bce93353525962f3e3f6447bc") # to get the data
library(EWStools)
data(EWStestData)
