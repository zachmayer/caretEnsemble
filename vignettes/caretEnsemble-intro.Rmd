...
As you can see from this plot, these 2 models are un-correlated, and the rpart model is ocassionally anti-predictive, with a one re-sample showing AUC of 0.46.
...
These 2 models make a good candidate for an ensemble: their predicitons are fairly un-correlated, but their overall accuracy is similar.  We do a simple, linear greedy optimization on AUC using caretEnsemble:
```{r, echo=TRUE}
