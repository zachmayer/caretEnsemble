...
Use the `methodList` argument to specify which models to fit: either a character vector specifying the model name or a `caretModelSpec` which can be used to customize the call to each component model and will be discussed in more detail later. First, lets build an example dataset (adapted from the caret vignette):
